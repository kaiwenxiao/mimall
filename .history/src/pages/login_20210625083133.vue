...
export default {
      ...
      methods: {
            login() {
                  let { username, password } = this; //解构语法（省略变量声明）
                  this.axios
                        .post("/user/login", {
                              // 传参时key value一致时可以简写
                              username,
                        ...
                        .then(res => {
                              ...
                              // 1.组件派发方法到vuex的action中
                              // this.$store.dispatch('saveUserName', res.username);
                              // 2.mapAction转发到vuex的action中
                              this.saveUserName(res.username);
                        }).catch(() => {
                              this.$message.warning("请输入正确的账号密码");
                        })
                        .then(() => {
                              this.axios.get("/npcarts/products/sum")
                                    .then(res => {
            ...
            // ]).then(
            //       this.axios.spread((loginRes, cartsumRes) => {
            //             this.$cookie.set('userId', loginRes.id, { expires: 'Session' });
            //             this.saveUserName(loginRes.username);
            //             this.saveCartCount(cartsumRes.data);
            //             this.$router.push({
            //                   name: 'index',
            //                   params: {
            //                         from: 'login'
